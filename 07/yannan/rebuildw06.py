...
def add():
    ...
    if not user or not pwd:
        sql_comm = 'select * from userinfo'
        return '<p>need user and pwd</p>'+render_template("index.html",data=conn_db.mysql_select(sql_comm))
    elif user_exist:
        sql_comm = 'select * from userinfo'
        return '<p>user exist</p>'+render_template("index.html",data=conn_db.mysql_select(sql_comm))
    else:
...
def delete():
    user = request.args.get('user')
    #sql_comm = 'select username from userinfo where username = %s' %(user)
    #if mysql_db.mysql_select(sql_comm) != '':
        #return render_template("index.html",data=mysql_db.mysql_select(sql_comm))
    #else:
